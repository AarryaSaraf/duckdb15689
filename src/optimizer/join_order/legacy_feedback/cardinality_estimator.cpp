...
// Set DUCKDB_JOIN_ORDER_DEBUG_LOG=1 to append one diagnostic line per multi-rel join (feedback_legacy binary).
static bool JoinOrderCardinalityDebugLogEnabled() {
	const char *v = std::getenv("DUCKDB_JOIN_ORDER_DEBUG_LOG");
...
double CardinalityEstimator::EstimateCardinalityWithSet(JoinRelationSet &new_set) {
	...
	// #region agent log
	if (log_file.is_open() && new_set.count >= 2 && JoinOrderCardinalityDebugLogEnabled()) {
		log_file << "DUCKDB_JOIN_DEBUG binary=join_order_feedback_legacy nrels=" << static_cast<uint64_t>(new_set.count)
		         << " relset=" << new_set.ToString()
		         << " injected=" << (has_injected ? "1" : "0")
		         << " note=cpp_logs_LOGICAL_JOIN_only_sql_count_query_not_emitted_oracle_py_expects_SQL_COUNT_QUERY"
		         << std::endl;
	}
	// #endregion
	log_file.close();
